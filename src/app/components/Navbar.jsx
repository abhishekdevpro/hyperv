  ...
  const NavItem = ({ href, label, isActive }) => (
    <Link 
      ...
      className={`nav-link transition-all duration-300 px-4 py-2 rounded-md ${
        isActive 
          ? `font-bold ${!scrolled ? "bg-white/20 text-white" : "bg-black/10 text-black"}` 
    ...
    >
      <div className="max-w-7xl mx-auto py-2 flex justify-between items-center px-4">
        ...
        <div className="hidden md:flex items-center space-x-2">
          ...
          <NavItem href="#products" label="Products" isActive={activePage === "products"} />
          <NavItem href="#about" label="About" isActive={activePage === "about"} />
          <NavItem href="#blog" label="Blog" isActive={activePage === "blog"} />
          <NavItem href="#contact" label="Contact" isActive={activePage === "contact"} />
        </div>

        {/* Desktop Social Icons */}
        <div className="hidden md:flex items-center space-x-4 text-2xl">
          <Link href="#" className={`social-icon hover:opacity-75 transition-opacity ${!scrolled ? "text-white" : ""}`}>
            <FaTwitter />
          </Link>
          <Link href="#" className={`social-icon hover:opacity-75 transition-opacity ${!scrolled ? "text-white" : ""}`}>
            <FaDiscord />
      ...
      {mobileMenuOpen && (
        <div className="md:hidden absolute top-full left-0 right-0 bg-white shadow-md py-4 px-4 flex flex-col space-y-2">
          <Link 
            href="/" 
            className={`nav-link py-2 px-3 rounded-md ${activePage === "home" ? "font-bold bg-black/5" : ""}`}
            onClick={() => setMobileMenuOpen(false)}
          >
            Home
          </Link>
          <Link 
            href="/products" 
            className={`nav-link py-2 px-3 rounded-md ${activePage === "products" ? "font-bold bg-black/5" : ""}`}
            onClick={() => setMobileMenuOpen(false)}
          >
            Products
          </Link>
          <Link 
            href="/about" 
            className={`nav-link py-2 px-3 rounded-md ${activePage === "about" ? "font-bold bg-black/5" : ""}`} 
            onClick={() => setMobileMenuOpen(false)}
          >
            About
          </Link>
          <Link 
            href="/blog" 
            className={`nav-link py-2 px-3 rounded-md ${activePage === "blog" ? "font-bold bg-black/5" : ""}`}
            onClick={() => setMobileMenuOpen(false)}
          >
            Blog
          </Link>
          <Link 
            href="/contact" 
            className={`nav-link py-2 px-3 rounded-md ${activePage === "contact" ? "font-bold bg-black/5" : ""}`}
            onClick={() => setMobileMenuOpen(false)}
          >
            Contact
          </Link>
          ...
          {/* Mobile Social Icons */}
          <div className="flex items-center space-x-4 py-2">
            <Link href="#" className="social-icon text-black hover:opacity-75 transition-opacity">
              <FaTwitter />
            </Link>
            <Link href="#" className="social-icon text-black hover:opacity-75 transition-opacity">
              <FaDiscord />
